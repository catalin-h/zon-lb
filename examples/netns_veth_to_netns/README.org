...
In the zone1 netns launch a =netcat= TCP server listening on port 223:
...
In the =zone0= netns run the =telnet= connecting to the pair veth IPv4
...
from the =zone1= netns without errors it will print:
